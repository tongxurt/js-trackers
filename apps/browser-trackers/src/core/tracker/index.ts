...
export function Tracker(
    ...
    const newTracker = (
        ...
        let // Tracker core
            ...
            configPostPath = trackerConfiguration.postPath ?? '/tracking-event',
            // Site ID
    ...
    const partialTracker = newTracker(trackerId, namespace, version, endpoint, sharedState, trackerConfiguration),
        tracker = {
            ...
            addPlugin: (configuration: BrowserPluginConfiguration) => {
                ...
                configuration.plugin.activateBrowserPlugin?.(tracker as BrowserTracker);
            },
        } as BrowserTracker;

    // Initialise each plugin with the tracker
    browserPlugins.forEach((p) => {
        p.activateBrowserPlugin?.(tracker as BrowserTracker);
    });
